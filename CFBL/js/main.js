...
function compileFunction(tokens, name, args, argKeys) {
    switch (tokens[0]) {
        ...
        case "win.setKey":
            if (args.length == 3)
                return `${compileValue(args[0], argKeys[0])}${compileValue(args[1], argKeys[1])}${compileValue(args[2], argKeys[2])}win setData ${argKeys[0]} ${argKeys[1]} ${argKeys[2]}\n`;
            break;
...
console.log(compileScript(code));